...
public class UserService extends BaseService<UserBean,String> {
    ...
    public void resetPassword(String userId) {
        ...
        //2、设置默认密码
        userBean.setPassword(encryptPassword(userBean.getUsername(),CommonConst.INITIAL_PASSWORD));
        save(userBean);
    }

    public void runAs(String ... ids) {
        Subject subject= SecurityUtils.getSubject();
        if(subject.isRunAs()){
            throw MessageDefine.ERROR_HAS_RUN_AS.toRuntimeException();
        }
        List<UserBean> userBeanList= findAll(new StringCondition("id",ids, StringCondition.Handler.IN));
